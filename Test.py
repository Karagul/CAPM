...
def main():
    ...
    for i in testtiny:
        ...
        stock = quandl.get("WIKI/" + i + '.11', start_date='2017-12-21', end_date='2017-12-31', returns='numpy')
...
def raw_data(list_of_stocks_data, list_of_stocks_name,number_of_stocks,exp_port_ret,is_short_sell):
    ...
    for i in range(len(r_vect)):
        for j in range(len(r_vect)):
            ...
            cov_matrix[i][j] = np.mean(ij)-np.mean(list_of_stocks_return[i])*np.mean(list_of_stocks_return[j])#np.cov(list_of_stocks_return[i],list_of_stocks_return[j])[0][1]
